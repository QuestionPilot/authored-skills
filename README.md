...
| Skill | What it does |
|---|---|
| **cross-model-review** | Routes "check / review / audit my work" to a different model (Codex) so the author never reviews its own output. Includes an outbound credential-scan gate before any external pipe. |
...
- **cross-model-review** expects the `codex` (and optionally `agy`) CLIs on `PATH`; its run-output directory is configurable via `CROSS_MODEL_OUT_DIR`.
