...
#
# Sourced by the cross-model-review skill before each `codex exec` /
# `agy` pipe. Lives in the operator-local skill dir.
#
# Contract (Codex review F-3/F-4/F-5 amendments, 2026-05-25):
...
scan_outbound() {
  # F-3: argument-count guard BEFORE reading $1 — under `set -u`, reading
  # $1 with no args would abort the calling script with an unbound-variable
  # error rather than returning the documented contract code.
  if [ "$#" -lt 1 ]; then
  ...
  # F-5: input must exist AND be readable. Either failure = fail closed.
  if [ ! -f "$input" ]; then
  ...
  local sk_a="sk-"; local sk_b="ant-"
  local xox_a="xox"; local xox_b='[bap]-'
  local gh_a="gh"; local gh_b='[pousr]_'
  local aws_a="AKIA"

  local patterns=(
    "${sk_a}${sk_b}[A-Za-z0-9_-]{40,}"      # Claude API keys
    "${xox_a}${xox_b}[A-Za-z0-9-]{30,}"     # Slack tokens
    "${gh_a}${gh_b}[A-Za-z0-9_]{36,}"       # GitHub tokens
    "${aws_a}[0-9A-Z]{16}"                  # AWS access keys
  ...
  # F-5: if grep is missing entirely, fail closed — we cannot prove the
  # input is clean.
  ...
  for p in "${patterns[@]}"; do
    # F-5: capture grep's exit code explicitly. grep -E returns:
