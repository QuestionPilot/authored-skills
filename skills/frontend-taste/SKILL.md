...
license: Apache 2.0. Incorporates Anthropic's frontend-design skill and taste-skill (MIT) mechanics. See NOTICE.md for attribution.
allowed-tools:
  - Bash(npx impeccable *)
---
...
## Anti-slop mechanics (taste-skill lineage)

The shared design laws above set direction; these mechanics make "how bold, how kinetic, how dense, and what to never ship" explicit and checkable. They are folded in from taste-skill (MIT — see NOTICE.md) and apply most directly to landing pages, portfolios, and marketing/redesign work. Pull each reference only when the task calls for it — they extend, not replace, the register references and command flows below.

**Set the three dials first.** Read the brief, then commit to explicit 1–10 values and say why; do not silently use the baseline:

- `DESIGN_VARIANCE` (symmetry → chaos), `MOTION_INTENSITY` (static → cinematic), `VISUAL_DENSITY` (airy → packed). Baseline `8 / 6 / 4` for a default landing build. Every layout, motion, and density call downstream is gated by these. Full inference tables + level definitions: [reference/dials.md](reference/dials.md).

**Then load what the task needs:**

- [reference/design-systems.md](reference/design-systems.md) — when the brief reads like an established product family (enterprise / Material / Fluent / Carbon / Shopify / Atlassian / GitHub / public-sector), reach for the official package instead of hand-rolling its look. Install commands + canonical docs included.
- [reference/scroll-motion.md](reference/scroll-motion.md) — canonical GSAP sticky-stack / horizontal-pan / scroll-reveal skeletons, forbidden animation patterns, and performance / reduced-motion guardrails. Read before building any scroll-driven motion.
- [reference/ai-tells.md](reference/ai-tells.md) — the granular landing-page tell catalog (extends the absolute bans above): the total em-dash ban, eyebrow restraint, fake-screenshot ban, decoration strips, locale strips, premium-consumer palette ban, serif discipline.
- [reference/redesign.md](reference/redesign.md) — for existing sites: mode detection (preserve vs overhaul), audit-first, modernisation levers, and what never changes silently (URLs, nav labels, form fields, SEO).
- [reference/pre-flight.md](reference/pre-flight.md) — the mechanical, countable check to run before delivering a landing / marketing build. Most boxes are grep-verifiable.
...
<!-- Operator-local skill. Renamed in place from `impeccable` to `frontend-taste`: consolidates the frontend-design lineage + taste-skill (MIT) mechanics. Internal machinery (npx impeccable CLI, .impeccable data dir, IMPECCABLE_* env/tokens) intentionally retains the impeccable identity; only the skill identity + model-facing paths were renamed. See NOTICE.md for attribution. -->